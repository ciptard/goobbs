...
{
	return (isModerator() || isAuthor($author)? '<a href = "edit.php?topic=' .$topic. '">[!]</a><a href = "delete.php?topic=' .$topic. '">[x]</a>' : '').hook('manageTopic', $topic);
}

function manageReply($reply, $author)
{
	return (isModerator() || isAuthor($author)? '<a href = "edit.php?reply=' .$reply. '">[!]</a><a href = "delete.php?reply=' .$reply. '">[x]</a>' : '').hook('manageReply', $reply);
}
